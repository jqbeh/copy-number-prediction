...
def main():
    ...
    # Map PE reads to genome using bwa mem
    run_command(
    ...
    # Convert CDS features from GFF/GFF3 to BED
    run_command(
        f"""awk -F'\\t' 'BEGIN{{OFS="\\t"}}
        /^##FASTA/ {{exit}}
        /^#/ {{next}}
    ...
    # Check if BAM and BED contig names overlap
    # If not, bedtools coverage would report 0 for every gene.
    validate_contigs_match(bam, bed)

    # Calculate mean depth per gene
    run_command(
        f"bedtools coverage "
        f"-a {bed} "
        f"-b {bam} "
